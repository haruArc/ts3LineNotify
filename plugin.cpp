...
const char* ts3plugin_name() {
	...
	if(!result) {
        const wchar_t* name = L"ts3LineNotify";
		if(wcharToUtf8(name, &result) == -1) {  /* Convert name into UTF-8 encoded result */
            strcpy(result, "ts3LineNotify");  /* Conversion failed, fallback here */
		}
	}
	return result;
#else
    return "ts3LineNotify";
#endif
}
...
int ts3plugin_init() {
    ...
    translator.load(QLatin1String("ts3LineNotify_" )+currentLang, QString().fromStdString(path));
...
void ts3plugin_configure(void* handle, void* qParentWidget) {
    ...
    LineNotifyConfigDialog qConfigDialog((QWidget*)qParentWidget);
    qConfigDialog.exec();
}
...
int ts3plugin_onTextMessageEvent(uint64 serverConnectionHandlerID, anyID targetMode, anyID toID, anyID fromID, const char* fromName, const char* fromUniqueIdentifier, const char* message, int ffIgnored) {
    ...
    if ((fromID != myID || Globals::sendToMyself) && hash[serverConnectionHandlerID]) {
        ...
        request.setRawHeader("Authorization",  QByteArray::fromStdString(QString("Bearer %1").arg(Globals::lineToken).toStdString()));
...
void ts3plugin_onMenuItemEvent(uint64 serverConnectionHandlerID, enum PluginMenuType type, int menuItemID, uint64 selectedItemID) {
    QApplication::instance()->installTranslator(&translator);
    LineNotifyConfigDialog qConfigDialog;
